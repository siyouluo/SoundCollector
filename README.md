...
# 咪头
咪头是将声音信号转换为电信号的设备, 这里选用电容式驻极体咪头.
...
# 小信号放大
这里选用两级LM358对小信号进行放大, 并通过电位计调节增益,
[Multisim 仿真](./MultisimProject)时, 该电路将幅度10mV,
频率1KHz的正弦交流电放大到了幅度为1V, 直流偏置为2.47V的正弦波.
实际使用时, 要考虑咪头输出电压大小, 调节电位计,
并在后级接入加减运算电路和比例运算电路, 将输出电压控制在
0~3.3V之间, 可能需要在AD采样时加入保护电路, 防止声音过大时烧坏MCU.  
更多[参考资料](./Reference/SignalAmplifier/readme.md)  

# 滤波

# AD转换
这里选用stm32f103c8t6作为主控芯片, 这款芯片内置2个12位ADC转换单元,
...
2019/3/5:
- [x] 完成小信号放大电路的设计与仿真, 但后续还要将输出信号调整到0~3.3V之间
